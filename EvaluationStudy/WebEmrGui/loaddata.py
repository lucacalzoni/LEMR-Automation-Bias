# crisma server
# Py3-compatible relative imports (see views.py for rationale).
...
def load_bp(case_id, time_cut, rollnames):
    ...
    diastolic = {"name": "dias", "color": "#5a5a5a", "data": curr_data[0], "marker": {"symbol": "circle"}}
    systolic = {"name": "syst", "color": "#2e2e2e", "data": curr_data[1], "marker": {"symbol": "circle"}}
    art_diastolic = {"name": "art_dia", "color": "#5a5a5a", "data": curr_data[2], "marker": {"symbol": "circle"}}
    art_systolic = {"name": "art_sys", "color": "#2e2e2e", "data": curr_data[3], "marker": {"symbol": "circle"}}
...
def load_local_report(case_id, time_cut, report_type, icu_admit_t, rare_type):
    ...
    if report_type in ['RAD', 'EKG']:
        detail_file = 'D:/MARS_pull/note/note_details_3_rad.txt'
        note_dir = 'D:/MARS_pull/rad_ekg_processed_notes/'
    else:
        detail_file = 'D:/MARS_pull/note/note_details_3.txt'
        note_dir = 'D:/MARS_pull/all_processed_notes/'
...
def load_case_date(case_id, out_folder='all/', time_cut=1451688581000):
    ...
    def process_data_into_series(x_, y_, p_norm_ranges, d_x, d_y):
        ...
        # Here colors for the graphs are defined. I replaced the original colors as noted in the comments.
        if p_norm_ranges[0] is not None and p_norm_ranges[1] is not None:  # was high values red & low values blue
            numeric["zones"].append({"value": p_norm_ranges[0], "color": "#006ddb"})  # now different shade of blue, was blue 00CCFF
            numeric["zones"].append({"value": p_norm_ranges[1], "color": "#000000"})  # now black, was green 33CC33
            numeric["zones"].append({"color": "#ffff6d"})  # now yellow, was red BF0B23
        elif p_norm_ranges[0] is None and p_norm_ranges[1] is not None:  # was high values red & rest green
            numeric["zones"].append({"value": p_norm_ranges[1], "color": "#000000"})  # now black, was green 33CC33
            numeric["zones"].append({"color": "#ffff6d"})  # now yellow, was red BF0B23
        elif p_norm_ranges[0] is not None and p_norm_ranges[1] is None:  # was low values blue & rest green
            numeric["zones"].append({"value": p_norm_ranges[0], "color": "#006ddb"})  # now different shade of blue, was blue 00CCFF
            numeric["zones"].append({"color": "#000000"})  # now black, was green 33CC33
...
def update_cases():
    ...
    # '''
    # load_case_date('demo-case-01', 'cases_t1/demo-case-01/')
