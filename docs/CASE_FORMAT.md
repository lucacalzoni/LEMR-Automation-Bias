...
The repo ships one minimum-viable synthetic case at
`EvaluationStudy/models/evaluation_study/data/demo-intro/`, generated by
`EvaluationStudy/scripts/generate_synthetic_case.py`. It's a fictitious ARF
patient with 3 days of 8 labs and 4 vitals — enough for reviewers to see the
interface render end-to-end, but nowhere near a full clinical picture. Extend
the generator's `LABS`, `VITALS`, and med lists to add more series, or call
`build_case('demo-case-02')` with a different ID to add cases. **The pickle files that drove the actual study cannot be
